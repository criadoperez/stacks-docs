...
#### description: 
Returns the _current_ stacking information for `stacker.  If the information\nis expired, or if there's never been such a stacker, then returns none.
...
#### description: 
Returns the absolute minimum amount that could be validly Stacked (the threshold to Stack in\na given reward cycle may be higher than this
